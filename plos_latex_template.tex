...
\subsection*{Spatial configuration of seurons allows for the precise prediction of firing Rates}\label{Section_Rand_Mat_vs_Sim}
...
\begin{equation}
nNOS_i = \frac{{Ca^{2+}_{spike}}^3 \tau_{Ca^{2+}}ln(2)}{3} \cdot r_i \equiv \gamma r_i \; .
\label{simpl_nNOS}
\end{equation}
...
\begin{align}
0 &=-\lambda NO + D \nabla^2 NO + \sum_{i} \delta^2(\mathbf{x}-\mathbf{x}_{neur,i})\cdot \gamma \cdot r_i \label{simple_NO_dyn_with_diff} \\
...
\begin{equation}
NO(\mathbf{x}) = \frac{r_i \gamma}{2 \pi D} K_0 \left(|\mathbf{x}|\sqrt{\frac{\lambda}{D}} \right) \equiv r_i \cdot \psi_{point}(|\mathbf{x}|)
...
By defining
\begin{equation}
\psi_{ij} \equiv \psi_{ij} \equiv \psi (|\mathbf{x}_{neur,i}-\mathbf{x}_{neur,j}|)
...
\begin{center}
\includegraphics[width=0.5\textwidth]{../../plots/scatter_density_out_weights.png}
%% density_vs_sum_out_weights_points.py
\includegraphics[width=0.5\textwidth]{../../plots/scatter_density_out_weights_points.png}
\end{center}
...
\begin{equation}
\langle nNOS \rangle = \frac{{Ca^{2+}_{spike}}^3 \tau_{Ca^{2+}}ln(2)}{3\tau_{nNOS}} \langle r \rangle
